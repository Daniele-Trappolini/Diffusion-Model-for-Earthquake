...
def train_val_test_split(df, train_percentage=0.70, val_percentage=0.10, test_percentage=0.20, force_in_test=[], plot_hist=True):
...
class FCN(nn.Module):
    ...
    def forward(self, batch_init, batch_size, steps_in):
        x = self.layer_1(batch_init.view(batch_size,3,steps_in))
        x = self.relu(x)
        x = self.dropout(x)
        x_1=x.clone()
...
class CNN(nn.Module): #This model is inspired by Temporal earthquake forecasting by Ong et al.
    ...
    def forward(self, batch_init, batch_size, steps_in, softmax=False): 
        x = self.relu(self.maxpool(self.BatchNorm1(self.conv1(batch_init.view(batch_size,3,steps_in))))) 
        x_1=x.clone()
...
def compute_saliency_maps(X, y, model, mdl, inp_size, saliency_target="ALL", s_max=False):
  """
...
def show_saliency_maps(X, y, model, mdl, inp_size, saliency_target="ALL", s_max=False,show_spectrogram=False):
  """
  ...
  tot_post=0
  wrong_post=0
  saliency,out = compute_saliency_maps(X, y, model, mdl, inp_size,saliency_target=saliency_target, s_max=s_max)
  ...
  for i in range(0,N):
    ...
    print("output=",out[i])
    print("label=",y[i])
    print("binary output=",to_one_hot(out[i].detach().numpy()))
    tot=tot+1
    if not torch.all(to_one_hot(out[i].detach().numpy()).type(torch.IntTensor).eq(y[i])):
      wrong=wrong+1
    if y[i][0]==1: #post
      tot_post=tot_post+1
      if not torch.all(to_one_hot(out[i].detach().numpy()).type(torch.IntTensor).eq(y[i])):
